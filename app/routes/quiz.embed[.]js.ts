...
import { fetchRequestHandler } from "@react-router/express";

export async function loader({ request }: LoaderFunctionArgs) {
  return new Response(getQuizEmbedCode(), {
    headers: {
...
function getQuizEmbedCode(): string {
  return `
class AIQuizWidget {
  constructor(options = {}) {
  ...
  render() {
    ...
    this.element.innerHTML = \`
      <style>
        #ai-quiz-container * { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }
        #ai-quiz-trigger {
          position: fixed; \${pos.top || pos.bottom ? (pos.top ? 'top: ' + pos.top : 'bottom: ' + pos.bottom) : 'bottom: 20px'};
        ...
        #ai-quiz-header { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; text-align: center; }
        ...
        .quiz-option.selected { background: #e8f4fd; border-color: #667eea; color: #667eea; }
        ...
        #quiz-submit-email { width: 100%; padding: 12px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; }
        ...
        #ai-quiz-next { padding: 10px 24px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 20px; font-size: 14px; font-weight: 600; cursor: pointer; opacity: 0.5; transition: opacity 0.2s; }
        ...
        .quiz-product-card p { margin: 0 8px 8px; font-size: 13px; font-weight: 600; color: #667eea; }
        .quiz-product-card a { display: block; margin: 0 8px 8px; padding: 6px; background: #667eea; color: white; text-align: center; border-radius: 4px; text-decoration: none; font-size: 11px; }
  ...
  bindEvents() {
    document.getElementById('ai-quiz-trigger')?.addEventListener('click', () => this.open());
    document.getElementById('ai-quiz-close')?.addEventListener('click', () => this.close());
    document.getElementById('ai-quiz-next')?.addEventListener('click', () => this.nextQuestion());
    document.getElementById('quiz-submit-email')?.addEventListener('click', () => this.submitEmail());
    document.getElementById('quiz-restart')?.addEventListener('click', () => this.restart());
    this.showQuestion(0);
  }

  open() { document.getElementById('ai-quiz-window')?.classList.add('active'); this.isOpen = true; }
  close() { document.getElementById('ai-quiz-window')?.classList.remove('active'); this.isOpen = false; }
  ...
  showQuestion(index) {
    ...
    document.getElementById('ai-quiz-question').textContent = question.question;
    document.getElementById('ai-quiz-progress-bar').style.width = ((index / this.quiz.questions.length) * 100) + '%';

    const options = question.options || [];
    document.getElementById('ai-quiz-options').innerHTML = options.map((opt, i) =>
      '<button class="quiz-option" data-index="' + i + '" data-value="' + opt.text + '">' + opt.text + '</button>'
    ).join('');

    document.getElementById('ai-quiz-options').querySelectorAll('.quiz-option').forEach((btn) => {
  ...
  showEmailCapture() {
    document.getElementById('ai-quiz-progress-bar').style.width = '100%';
  ...
  async submitEmail() {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) this.showResults(data.products || []);
...
window.addEventListener('DOMContentLoaded', () => {
  const container = document.getElementById('ai-quiz-widget');
  if (container) new AIQuizWidget({ shop: container.getAttribute('data-shop'), apiBase: '', position: 'bottom-right' });
});
`.trim();
  }
}
